...
const httpserver = http.createServer((req, res) => {
  ...
  if (req.url.startsWith("/cdn/image.gif")) {
    ...
    perfbeacon(req,res);
    res.end(image1x1);
  } else if (req.url.startsWith("/cdn/pushstates")) {
    if (req.headers['subscribe-key'] != cdnboxStates.subscribekey) {
      res.writeHead(401, { 'Content-Type': 'text/plain' });
    } else {
      req.on('data', (data) => { cdnboxStates.putstatevector(data); });
    }
    res.end();
    ...
    if (subscribestate && cdnboxStates[subscribestate] !== undefined) {
      cdnboxStates.subscribestate[subscribestate] = { "secretkey": req.headers['subscribe-key'] };
    }
  ...
  // acces limite.
    ...
    if (req.method === 'OPTIONS') {
      ...
      // UI (raw).
        ...
        res.writeHead(200, { "Content-Type": "text/html", "Refresh": "5", "Access-Control-Allow-Origin": "*" });
...
function htmlcdnboxes(req,res) {
  ...
  if (cdnboxStates.local.config.ishttp) {
      if (cdnboxStates.local.status != "off") {
        res.write('<a href="/cdn/cdnboxes/action=off">Off</a>');
      } else if (cdnboxStates.local.status == "off") {
        res.write('<a href="/cdn/cdnboxes/action=on">On</a>');
      }
  }
  res.write('</span>');
  ...
  res.write(
      ...
      '<div class="head head2"><span class="release">' +
      cdnboxStates.release + '</span><span class="action">' +
      '<span class="dnscount">DNS: ' + cdnboxStates.globaldnscount.last + '/' +
      Math.round(cdnboxStates.globaldnscount.average) + '/' +
      Math.round(cdnboxStates.globaldnscount.limit) + '</span></div>' +
      '<table><tr><th>Name</th><th>BW</th><th>Trend</th><th>Perf</th><th>Score</th>' +
      '<th>Bu.</th><th>Penal</th><th>HTTP</th><th>DNS</th></tr>'
  );
  for (var i=0; i < cdnboxStates.config.cdnboxes.length; i++ ) {
    ...
    res.write(
        ...
        '<td class="number">' +
        (cdnboxState.config.penal !== undefined?
            Math.floor(cdnboxState.config.penal + cdnboxStates.penalbucket.sigma / 4):'-') +
        '</td>' +
        '<td class="status">' +
        ...
        '</td>' +
        '<td class="status">' + (cdnboxState.config.isns?'yes':'no') + '</td></tr>'
    );
  }
  res.write('</table>\n</body>\n</html>');
}

function htmlconsole(res) {
  ...
  for (var i=0; i < cdnboxStates.config.cdnboxes.length; i++ ) {
    ...
    if (cdnboxState.config.isns) {
      res.write(
          ...
          cdnboxState.config.proto + '//' + cdnboxState.config.hostname + '/cdn/cdnboxes"></iframe>\n'
      );
    }
  }
  res.write('\n</body>\n</html>');
...
process.on('uncaughtException', (err) => {
  if (err.code == "ECONNRESET" && err.errno == "ECONNRESET" && err.syscall == "read") {
    logerror('"error": "uncaughtException ECONNRESET"');
  } else {
