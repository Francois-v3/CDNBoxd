...
var tcp6dnsserver = dnsserver.createTCPServer();
tcp6dnsserver.on('request', function (request, response) { dnsrequest(request, response); } );
tcp6dnsserver.on('error', function (err, buff, req, res) { console.error(err.stack); });
tcp6dnsserver.on('listening', () => {
    console.error("DNS TCP6 server launched on port %s", cdnboxStates.config.dnsserver.port);
});

var cdnboxDNSserver = module.exports = function () { };

cdnboxDNSserver.serve = function (choose, ip4addr) {
console.error('serveIP4: ' + ip4addr);
  cdnboxDNSserver.choose = choose;
  calcdnscount();
  timers.setInterval(calcdnscount, 1000);
  udpdnsserver.serve(cdnboxStates.config.dnsserver.port, ip4addr);
  tcpdnsserver.serve(cdnboxStates.config.dnsserver.port, ip4addr);
}

cdnboxDNSserver.serveIPv6 = function (ip6addr) { 
  udp6dnsserver.serve(cdnboxStates.config.dnsserver.port, ip6addr); 
  tcp6dnsserver.serve(cdnboxStates.config.dnsserver.port, ip6addr); 
}
  ...
  if (reqname.endsWith(cdnboxStates.config.dnsserver.domain)) {
    ...
    if ((req.type == dnsconsts.NAME_TO_QTYPE.NS || req.type == dnsconsts.NAME_TO_QTYPE.ANY) && 
      ...
      for (var i=0; i < cdnboxStates.config.dnsserver.nsnum && i < nslist.length; i++) {
        resp.push({ "type":dnsconsts.NAME_TO_QTYPE.NS,"class":1,"name": req.name, data: nslist[i] + '.',
    ...
    if (boxname != null) {
      ...
      if (cdnboxStates[boxname].ipv4 &&
        ...
        resp.push({ "type":dnsconsts.NAME_TO_QTYPE.A,"class":1, "name": req.name,
      ...
      if (cdnboxStates[boxname].ipv6 &&
        ...
        resp.push({ "type":dnsconsts.NAME_TO_QTYPE.AAAA,"class":1, "name": req.name,
      ...
      if (cdnboxStates[boxname].config.cname) {
        resp.push({ "type": dnsconsts.NAME_TO_QTYPE.CNAME,"class":1, "name": req.name,
