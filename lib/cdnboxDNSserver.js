...
class cdnboxDNSserver extends EventEmitter {
  ...
  handleMessage(msg, remoteaddress) {
    ...
    var request, buff, response = new NDPPacket();
    try {
      ...
      dnsrequest(request, response, remoteaddress.address);
      buff = new Buffer.allocUnsafe(response.payload);
...
// reponse DNS
function dnsrequest(request, response, remoteaddress) {
  ...
  if (reqname.endsWith(cdnboxStates.config.dnsserver.domain)) {
    ...
    var country = countryLookup.get(ednsadd?ednsadd:remoteaddress);
    ...
    console.log(' "type": "dns", "client": "' + remoteaddress + '", "ednsadd": "' + ednsadd +
